...
class FocusToYourController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (empty($requestData)) {
            return view('fyl/focusToYour.index', [
                'training' => $training,
                'trainingId' => 0,
            ...
            $focusToYourParticipants = DB::select('CALL get_fyl_focus_to_your_participants(?,?,?,?,?,?,?)', [$trainingId, $trainingIdEnroller, $search, $call_B, $call_L,$perfil,$mode]);
            $contador = 1;
            

            foreach ($focusToYourParticipants as $focusToYourParticipantsItem) {
                $focusToYourParticipantsItem->secuencial = $contador;
                $contador++;
            }

            
            $distinctTrainings = DB::table('fyl_team_enroller_for_training_attended')->where('training_id', $trainingId)->orderBy('name','ASC')->pluck('name','id');

            //return $distinctTrainings;
            ...
            $data = [
                ...
                'training' => $training,
                'focusToYourParticipants' => $focusToYourParticipants,
            ...
            return view('fyl/focusToYour.index', $data);
        }
    }
    ...
    public function create()
    {
        ...
        return view('fyl/focusToYour/create', [
            'city' => $city,
    ...
    public function store(Request $request)
    {
        ...
        return to_route('FocusToYour.index', ['training_id' => $trainingId])->with('status', 'Campus updated!');
    ...
    public function left_over(Request $request)
    {
       // return $request;
        ...
        return to_route('FocusToYour.index', [
    ...
    private function getYourParticipants($trainingId)
    {
        $focusParticipants = FocusParticipants::from('fyl_focus_participants as FP')
            ...
            ->select(
                ...
                DB::raw("(SELECT COUNT(*) FROM fyl_follow_focus AS Detail WHERE Detail.training_id = FP.training_id AND Detail.program_id = 3 AND Detail.participant_DNI = FP.participant_DNI) as calls"),
                DB::raw("(SELECT date_call FROM fyl_follow_focus WHERE training_id = FP.training_id AND program_id = 3 AND participant_DNI = FP.participant_DNI order by date_call desc, created_at desc  limit 1) as end_call"),
                DB::raw("(SELECT confirm_assistance FROM fyl_follow_focus WHERE training_id = FP.training_id AND program_id = 3 AND participant_DNI = FP.participant_DNI order by date_call desc, created_at desc  limit 1) as confirm_assistance"),
                'T.team_name',
                'P.nickname',
                'P.phone',
    ...
    public function edit($id)
    {
        ...
        $followUp = DB::table('fyl_follow_focus as FF')
        ...
        return view('fyl/focusToYour/edit', [
    ...
    public function call(Request $request)
    {
        ...
        if ($focusParticipants) {
            ...
            if ($consulta) {
                if ($request->type_call == 'Seguimiento') {
                    $focusParticipants->update([
                        'confirm_assistance_s' => $consulta->confirm_assistance,
                        'user_id_call_s' => $consulta->user_id,
                        'calls_s' => $consulta->cant
                    ]);
                } 
            }
        }
    ...
    public function destroy(FocusParticipants $focusParticipants)
    {
        try {
            ...
            return to_route('FocusToYour.index')->with('status', __('FocusToYour deleted!'));
        } catch (Exception $e) {
            return to_route('FocusToYour.index')->with('errors', 'LA llamada no puede ser eliminada.');
        }
    }
    ...
    public function team(Request $request)
    {
        ...
        $focusParticipantsEmpty = yourParticipants::from('fyl_focus_participants as FP')
            ->join('fyl_participants as P', 'FP.participant_DNI', '=', 'P.DNI')
            ->select(
                'FP.id',
                'FP.training_id',
        ...
        $focusParticipants = YourParticipants::from('fyl_focus_participants as FP')
            ->join('fyl_participants as P', 'FP.participant_DNI', '=', 'P.DNI')
            ->select(
                'FP.id',
                'FP.training_id',
        ...
        if (empty($requestData)) {
            return view('fyl/focusToYour.team', [
                'training' => $training,
