...
class FocusDashboardController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (empty($requestData)) {
            return view('fyl/focusParticipants.dashboard', [
                'training' => $training,
                'trainingId' => 0
            ]);
        } else {
            ...
            $payments = DB::select('CALL get_fyl_dashboard_focus_payments(?)', [$trainingId]);
            $payments_staff = DB::select('CALL get_fyl_dashboard_focus_payments_staff(?)', [$trainingId]);
            $confirm = DB::select('CALL get_fyl_dashboard_focus_confirm(?)', [$trainingId]);
            $attendance = DB::select('CALL get_fyl_dashboard_focus_attendance(?)', [$trainingId]);
            $attendance_teams = DB::select('CALL get_fyl_dashboard_focus_attendance_teams(?)', [$trainingId]);
            
            $gender = DB::select('CALL fyl_focus_participants_gender(?)', [$trainingId]); 
            $city = DB::select('CALL fyl_focus_participants_city(?)', [$trainingId]);
            $ageRange = DB::select('CALL fyl_focus_participants_age_range(?)', [$trainingId]);
