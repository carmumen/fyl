...
class TeamStaffController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $training = DB::table('fyl_next_training_focus_view as T')
        ...
        else
        {
            ...
            $legendaryDNI = $request->legendary_DNI;

            $staffDNI = $request->staff_DNI;

            $legendary = DB::table('fyl_legendary_focus_view')->where('training_id',$trainingId)->pluck('name','id');

            $staff = DB::table('fyl_staff_focus_view')->where('training_id',$trainingId)->pluck('name','id');
            ...
            $participants = DB::table('fyl_focus_participants_view')->where('training_id',$trainingId)->pluck('name','id');

            $staffDNI = $request->staff_DNI;

            $focusParticipants = DB::table('fyl_focus_participants_staff_view as FP')
            ...
            return view('fyl/focusParticipants.team', [
                ...
                'staff_legendary' => $staff_legendary,
                'team_staff_legendary' => $team_staff_legendary,
                'staffDNI' => $staffDNI,
                'staff' => $staff,
    ...
    public function store(Request $request)
    {
        try {
            ...
            $focus = DB::table('fyl_focus_participants')->where('id',$request->participants_DNI)->first();

            if ($focus) {
                DB::table('fyl_focus_participants')
    ...
    {
        $focus = DB::table('fyl_focus_participants')->where('id',$id)->first();
        if ($focus) {
            DB::table('fyl_focus_participants')
                ->where('id', $id)
    ...
    {
        $focus = DB::table('fyl_staff_focus')->where('id',$id)->first();
        if ($focus) {
            DB::table('fyl_staff_focus')
                ->where('id', $id)
