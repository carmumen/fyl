<x-app-layout>
    ...
    <header>
        <div class="flex flex-wrap justify-between mx-8">
                ...
                <nav class="space-x-4">
                    <span class="">
                        @if (isset($training) && count($training) > 0)
                            <form id="your" method="GET" class="flex items-center space-x-2"
                                action="{{ route('ReportsListYour.Listado') }}" enctype="multipart/form-data">
                                @csrf
            ...
            @if (isset($lista_your) && count($lista_your) > 0)
            
                <div class="p-1">
                    <a class=" {{ Config::get('style.btnSave') }} " target="_blank"
                        href="{{ url('/gafetes-your' . '/' . $trainingId) }}">Gafetes</a>
                </div>
            
            
                <div class="p-1">
                    <a class="{{ Config::get('style.btnSave') }}"
                        href="{{ url('/exportar-tabla' . '/listay' . '/' . $trainingId . '/y') }}">
                        Listado</a>
                </div>
            @endif
                ...
                <main class="border border-gray-200 md:rounded-lg">
                    <div class="overflow-x-auto">
                        <div id="conResultados">
                            ...
                            @include('fyl/reports.lista_your_table', $lista_your)
