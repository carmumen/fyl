<x-app-layout>
    @php
        if (session('entidad') == 'FocusToYour') {
            ...
            <tr>
                <td class="px-4" style="width: 20%">
                    @if (isset($training) && count($training) > 0)
                            <form id="your" method="POST" action="{{ route('FocusToYour.store') }}">
                                @csrf
                ...
                <td class="px-4" style="width: 20%">
                    @if (isset($focusToYourParticipants) && count($focusToYourParticipants) > 0)
                        ...
                        <table id="tablaDatos" class="min-w-full divide-y divide-gray-200">
                            ...
                            <thead class="bg-sky-800">
                                <tr>
                                    ...
                                    <th scope="col" class="{{ Config::get('style.headerCenterXs') }}">
                                        <button class="sort-button" data-sort="surnames">@lang('STATEMENT')</button>
                                    </th>
                                    <th scope="col" class="{{ Config::get('style.headerCenterXs') }}">
                                        <button class="sort-button" data-sort="surnames">@lang('PAGO<br>YOUR')</button>
                                    </th>
                                    <th scope="col" class="{{ Config::get('style.headerCenterXs') }}">
                                        <button class="sort-button" data-sort="surnames">@lang('PAGO<br>LIFE')</button>
                                    </th>
                                    ...
                                    <th scope="col"
                                        class="{{ Config::get('style.headerCenterXs') }} w-12 hidden sm:table-cell">
                                    ...
                                    <th scope="col"
                                        ...
                                        <button class="sort-button"
                                            data-sort="team_enroller">@lang('EQUIPO<br>ENROLADOR')</button>
                                    </th>
                                    <th scope="col"
                                        class="{{ Config::get('style.headerCenterXs') }} w-64  hidden sm:table-cell">
                                        <button class="sort-button"
                                            data-sort="team_enroller">@lang('ENROLADOR')</button>
                                    </th>
                                </tr>
                            </thead>
                            <tbody class="bg-gray-100">
                                @foreach ($focusToYourParticipants as $theFocusToYourParticipants)
                                    <tr class="border-b border-gray-200">
                                        <td class="{{ Config::get('style.rowSequential') }}">
                                            {{ $theFocusToYourParticipants->secuencial }}
                                        </td>
                                        ...
                                        @auth
                                            <td class="w-32 text-center py-1">
                                                <div
                                                    ...
                                                    @if ($theFocusToYourParticipants->statement_count == 0) rounded-md bg-red-500 py-2
                                                    @else
                                                        @if ($theFocusToYourParticipants->confirm_assistance_statement == 'SI') rounded-md bg-green-500 @endif
                                                        @if ($theFocusToYourParticipants->confirm_assistance_statement == 'NC') rounded-md bg-blue-500 @endif
                                                        @if ($theFocusToYourParticipants->confirm_assistance_statement == 'PF') rounded-md bg-orange-500 @endif
                                                        @if ($theFocusToYourParticipants->confirm_assistance_statement == 'PC') rounded-md bg-yellow-500 @endif
                                                        @if ($theFocusToYourParticipants->confirm_assistance_statement == 'NI') rounded-md bg-gray-500 @endif
                                                        @if ($theFocusToYourParticipants->confirm_assistance_statement == 'SLL') rounded-md bg-red-500 @endif
                                                    @endif">
                                                    <div>
                                                        <span>{{ $theFocusToYourParticipants->statement_count }}</span>

                                                        <form method="POST" class="{{ Config::get('style.btnCall') }}"
                                                            action="{{ route('FocusToYourCall.call') }}">
                                                            @csrf
                                                            <input type="hidden" name="id"
                                                                value="{{ $theFocusToYourParticipants->id }}" />
                                                            <input type="hidden" name="type_call" value="Seguimiento" />

                                                            <button
                                                                class="icon-phone-hang-up text-green-900 hover:bg-green-500 hover:text-white"
                                                                type="submit"></button>
                                                        </form>
                                                        <span>{{ $theFocusToYourParticipants->confirm_assistance_statement }}</span>
                                                    </div>
                                                    @php
                                                        $words = explode(' ', $theFocusToYourParticipants->userStatement);
                                                        $initials = '';
                                        ...
                                        <td class="{{ Config::get('style.rowCenterXs') }} text-xxs ">
                                            {{ $theFocusToYourParticipants->confirm_assistance_next }}
                                        </td>
                                        
                                        <td class="{{ Config::get('style.rowCenterXs') }} text-xxs ">
                                            {{ $theFocusToYourParticipants->payment_status_your }}
                                        </td>
                                        
                                        <td class="{{ Config::get('style.rowCenterXs') }} text-xxs ">
                                            {{ $theFocusToYourParticipants->payment_status_life }}
                                        </td>
                                        
                                        <td class="{{ Config::get('style.rowLeftXs') }} text-xxs ">
                                            {{ $theFocusToYourParticipants->participant }}
                                        </td>
                                        <td
                                            class="{{ Config::get('style.rowCenterXs') }} text-xxs hidden sm:table-cell">
                                            {{ $theFocusToYourParticipants->nickname }}
                                        </td>
                                        <td
                                            class="{{ Config::get('style.rowCenterXs') }} text-xxs hidden sm:table-cell">
                                            {{ $theFocusToYourParticipants->phone }}
                                        </td>
                                        <td
                                            class="{{ Config::get('style.rowCenterXs') }} text-xxs hidden sm:table-cell">
                                            <b>{{ $theFocusToYourParticipants->record_mode }}</b>
                                            {{-- @if ($theFocusToYourParticipants->number_your)
                                                <br>
                                                {!! nl2br(e($theFocusToYourParticipants->number_your)) !!}
                                            @endif --}}
                                        </td>
                                        <td
                                            class="{{ Config::get('style.rowCenterXs') }} text-xxs hidden sm:table-cell">
                                            {{ $theFocusToYourParticipants->team_enroller }}
                                        </td>
                                        <td
                                            class="{{ Config::get('style.rowCenterXs') }} text-xxs w-64 hidden sm:table-cell">
                                            {{ $theFocusToYourParticipants->enroller }}
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
