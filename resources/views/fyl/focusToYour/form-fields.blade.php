<div class="space-y-4">
    ...
    <label class="flex flex-col">
        ...
        <select class="{{ Config::get('style.cajaTexto') }}"
                type="text"
                name="city_id"
                value=" {{ old('city_id', $yourParticipants->city_id) }}" required/>
                <option value="">-- Seleccione --</option>
            @foreach($city->toArray() as $id => $name)
                <option value="{{ $id }}"
                    @if($id == old('city_id', $yourParticipants->city_id)) selected @endif
                >{{ __($name) }}</option>
            @endforeach
        </select>
    </label>
    ...
    <label class="flex flex-col">
        <span class="{{ Config::get('style.label') }}">@lang('yourParticipants')</span>
        <input class="{{ Config::get('style.cajaTexto') }}"
                type="text"
                name="name"
                value=" {{ old('name', $yourParticipants->name) }}" required/>
        @error('name')
    ...
    <label class="flex flex-col">
        ...
        <input class="{{ Config::get('style.cajaTexto') }}"
                type="text"
                name="address"
                value=" {{ old('address', $yourParticipants->address) }}" required/>
        @error('address')
    ...
    <label class="flex flex-col">
        ...
        <input class="{{ Config::get('style.cajaTexto') }}"
                ...
                value=" {{ old('phone', $yourParticipants->phone) }}" required/>
        @error('phone')
            <small class="pt-1 font-bold text-red-500/80">{{ $message }}</small>
        @enderror
    </label>
    ...
    <label class="flex flex-col">
        ...
        <select class="{{ Config::get('style.cajaTexto') }} "
                ...
                required/>
            <option value="ACTIVE"
                 @if($yourParticipants->status =="ACTIVE") selected @endif
            >@lang('ACTIVE')</option>
            <option value="INACTIVE"
                 @if($yourParticipants->status =="INACTIVE") selected @endif
