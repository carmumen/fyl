<x-app-layout>
    ...
    @php
        if (session('status')) {
            ...
            $trainingId = session('trainingId_y');
            $exists = session('exist_y');
        }
    @endphp
    <header>
        @if ($trainingId != 0)
    ...
    <div class="{{ Config::get('style.containerIndex') }}">
        ...
        <fieldset class=" border border-solid border-gray-300 py-2 rounded-md">
            <legend class="text-sm text-sky-900">@lang('Carga Staff')</legend>
            <div class="flex flex-row flex-wrap mx-4">
                <label class="flex flex-col py-1">
                    <form id="your" method="GET" action="{{ route('StaffYour.index') }}">
                        @csrf
                ...
                <label class="flex flex-col mx-2 mt-4 py-1">
                    @if (isset($exists) && $exists == 'NO')
                        <form id="massive" method="POST" action="{{ route('staff-yours.storeMassive') }}">
                            @csrf
                            <input type="hidden" name="training_id" value="{{ $trainingId }}" />
                            <button class="{{ Config::get('style.btnSave') }} mt-1" form="massive"
                                type="submit">Cargar Staff de Focus</button>
                        </form>
                    @endif
                </label>
            </div>
            @if ($trainingId != 0)
                <form id="form1" method="POST" action="{{ route('StaffYour.store') }}">
                    @csrf
        ...
        <fieldset id="newRegister" class=" border border-solid border-gray-300 py-2 rounded-md"
            ...
            <form id="form2" method="POST" action="{{ route('StaffYour.store') }}">
                @csrf
                                    ...
                                    <tr class="border-b border-gray-200">
                                        ...
                                        @auth
                                            <td class="w-24 inline-flex text-center py-1.5">
                                                <form action="{{ route('StaffYour.destroy', $thestaff->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            const btAgregar = document.querySelectorAll('btAgregar');
