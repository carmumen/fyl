<x-app-layout>
    ...
    <div class="flex flex-row">
        ...
        @if (isset($staff) && count($staff) > 0)
            <div class="{{ Config::get('style.containerIndex') }} ">
                <form method="POST" action="{{ route('TeamStaffYour.store') }}">
                    @csrf
                    <div class="flex flex-col md:flex-row py-1">
                            ...
                            <main class="mt-4 border border-gray-200 md:rounded-lg mb-10">
                                <div id="conResultados">
                                    ...
                                    <table id="tablaDatos" class="min-w-full divide-y divide-gray-200">
                                        ...
                                        <tbody class="bg-gray-100">
                                            @foreach ($yourParticipants as $theFocusParticipants)
                                                <tr class="border-b border-gray-200">
                                                    <td class="{{ Config::get('style.rowSequential') }} w-12 ">
                                                    ...
                                                    @auth
                                                        <td class="w-16 inline-flex text-center py-1">
                                                            @if ($theFocusParticipants->staff)
                                                                <form
                                                                    action="{{ route('TeamStaffYour.destroy', $theFocusParticipants->id) }}"
                                                                    method="POST">
                                                                    @csrf
                                                                    @method('DELETE')
...
<script>
    $(document).ready(function() {
        ...
        $('.legendary-select').on('change', function() {
            ...
            var updateUrl = "{{ route('TeamStaffYour.store') }}";
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    var url = '/TeamStaffYour?training_id=' + response;
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        function redirectToController() {
            ...
            if (trainingId) {
                var url = '/TeamStaffYour?training_id=' + trainingId;
                window.location.href = url;
            }
        }
