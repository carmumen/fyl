<x-app-layout>
    @php
        if (session('entidad') == 'YourParticipants') {
            ...
            session(['entidad' => 'YourParticipants']);
            session(['search' => '']);
        }
    @endphp
    <style>
                ...
                <tr>
                    <td class="text-center">
                        @if (isset($training) && count($training) > 0)
                            <form id="your" method="POST" action="{{ route('YourParticipants.index') }}">
                                @csrf
        ...
        <div class="w-full popupContainer">
            <div id="bienvenida" class="p-4 text-center">
                @if (isset($follow) && count($follow) > 0)
                    ...
                    @include('fyl/yourParticipants/tabla', [
                        ...
                        <table id="tablaDatos" class="min-w-full divide-y divide-gray-200">
                            ...
                            <thead class="bg-sky-800">
                                <tr>
                                    ...
                                    <th scope="col" class="{{ Config::get('style.headerCenterXs') }}">
                                        <button class="sort-button" data-sort="surnames">@lang('Participant')</button>
                                    </th>
                                    <th scope="col"
                                    ...
                                    <th scope="col"
                                        class="{{ Config::get('style.headerCenterXs') }} w-64  hidden sm:table-cell">
                                        <button class="sort-button"
                            ...
                            <tbody class="bg-gray-100">
                                @foreach ($yourParticipants as $theYourParticipants)
                                    <tr class="border-b border-gray-200"
                                        @if ($theYourParticipants->sunday_attended === 'ASISTIÓ') style="background-color:lightgreen" @endif
                                            @if ($theYourParticipants->sunday_attended === 'NO ASISTIÓ') style="background-color:lightyellow" @endif
                                            @if ($theYourParticipants->sunday_attended === 'DESERTÓ') style="background-color:lightpink" @endif>
                                        ...
                                        @auth
                                            <td class="w-32 text-center py-1">
                                                <div
                                                    ...
                                                    @if ($theYourParticipants->welcome_count == 0) rounded-md bg-red-500 py-2
                                                    @else
                                                        @if ($theYourParticipants->confirm_assistance_welcome == 'SI') rounded-md bg-green-500 @endif
                                                        @if ($theYourParticipants->confirm_assistance_welcome == 'NC') rounded-md bg-blue-500 @endif
                                                        @if ($theYourParticipants->confirm_assistance_welcome == 'PF') rounded-md bg-orange-500 @endif
                                                        @if ($theYourParticipants->confirm_assistance_welcome == 'PC') rounded-md bg-yellow-500 @endif
                                                        @if ($theYourParticipants->confirm_assistance_welcome == 'NI') rounded-md bg-gray-500 @endif
                                                        @if ($theYourParticipants->confirm_assistance_welcome == 'SLL') rounded-md bg-red-500 @endif
                                                    @endif">
                                                    <div>
                                                        <span>{{ $theYourParticipants->welcome_count }}</span>

                                                        <form method="POST" class="{{ Config::get('style.btnCall') }}"
                                                            action="{{ route('YourCall.call') }}">
                                                            @csrf
                                                            <input type="hidden" name="id"
                                                                value="{{ $theYourParticipants->id }}" />
                                                            <input type="hidden" name="type_call" value="Bienvenida" />

                                                            <button
                                                                class="icon-phone-hang-up text-green-900 hover:bg-green-500 hover:text-white"
                                                                type="submit"></button>
                                                        </form>
                                                        <span>{{ $theYourParticipants->confirm_assistance_welcome }}</span>
                                                    </div>
                                                    @php
                                                        $words = explode(' ', $theYourParticipants->userWelcome);
                                                        $initials = '';
                                            ...
                                            <td class="w-32 text-center py-1">
                                                <div
                                                    class="text-xs w-32 ml-1
                                                    @if ($theYourParticipants->logistics_count == 0) rounded-md bg-red-500 py-2
                                                    @else
                                                        @if ($theYourParticipants->confirm_assistance_logistics == 'SI') rounded-md bg-green-500 @endif
                                                        @if ($theYourParticipants->confirm_assistance_logistics == 'NC') rounded-md bg-blue-500 @endif
                                                        @if ($theYourParticipants->confirm_assistance_logistics == 'PF') rounded-md bg-orange-500 @endif
                                                        @if ($theYourParticipants->confirm_assistance_logistics == 'PC') rounded-md bg-yellow-500 @endif
                                                        @if ($theYourParticipants->confirm_assistance_logistics == 'NI') rounded-md bg-gray-500 @endif
                                                        @if ($theYourParticipants->confirm_assistance_logistics == 'SLL') rounded-md bg-red-500 @endif
                                                    @endif">
                                                    <div>
                                                        <span>{{ $theYourParticipants->logistics_count }}</span>
                                                        ...
                                                        <span>{{ $theYourParticipants->confirm_assistance_logistics }}</span>
                                                    </div>
                                                    @php
                                                        $words = explode(' ', $theYourParticipants->userLogistics);
                                                        $initials = '';
                                                        foreach ($words as $word) {
                                                            $initials .= strtoupper(substr($word, 0, 1));
                                                        }
                                        ...
                                        <td class="{{ Config::get('style.rowCenterXs') }}" title="ÚLTIMO SEGUIMIENTO">
                                            {{ $theYourParticipants->confirm_assistance_up }}
                                        </td>
                                        <td class="{{ Config::get('style.rowCenterXs') }}">
                                                <select
                                                    ...
                                                    <option value="NO ASISTIÓ"
                                                        @if ($theYourParticipants->friday_attended === 'NO ASISTIÓ') selected @endif><b>NO
                                                            ASISTIÓ</b>
                                                    </option>
                                                    <option value="ASISTIÓ"
                                                        @if ($theYourParticipants->friday_attended === 'ASISTIÓ') selected @endif>
                                                        <span style='color:red'>ASISTIÓ</span>
                                                    </option>
                                                    <option value="DESERTÓ"
                                                        @if ($theYourParticipants->friday_attended === 'DESERTÓ') selected @endif>
                                                        DESERTÓ</option>
                                                </select>
                                            </td>
                                            <td class="{{ Config::get('style.rowCenterXs') }}">
                                                <select
                                                    ...
                                                    data-participant-id="{{ $theYourParticipants->id }}">
                                                    <option value="NO ASISTIÓ"
                                                        @if ($theYourParticipants->saturday_attended === 'NO ASISTIÓ') selected @endif><b>NO
                                                            ASISTIÓ</b>
                                                    </option>
                                                    <option value="ASISTIÓ"
                                                        @if ($theYourParticipants->saturday_attended === 'ASISTIÓ') selected @endif>
                                                        <span style='color:red'>ASISTIÓ</span>
                                                    </option>
                                                    <option value="DESERTÓ"
                                                        @if ($theYourParticipants->saturday_attended === 'DESERTÓ') selected @endif>
                                                        DESERTÓ</option>
                                                </select>
                                            </td>
                                            <td class="{{ Config::get('style.rowCenterXs') }}">
                                                <select
                                                    ...
                                                    data-participant-id="{{ $theYourParticipants->id }}">
                                                    <option value="NO ASISTIÓ"
                                                        @if ($theYourParticipants->sunday_attended === 'NO ASISTIÓ') selected @endif><b>NO
                                                            ASISTIÓ</b>
                                                    </option>
                                                    <option value="ASISTIÓ"
                                                        @if ($theYourParticipants->sunday_attended === 'ASISTIÓ') selected @endif>
                                                        <span style='color:red'>ASISTIÓ</span>
                                                    </option>
                                                    <option value="DESERTÓ"
                                                        @if ($theYourParticipants->sunday_attended === 'DESERTÓ') selected @endif>
                                                        DESERTÓ</option>
                                                </select>
                                            </td>
                                        ...
                                        <td
                                            ...
                                            @if($theYourParticipants->phone2)
                                                <br>
                                                {{ $theYourParticipants->phone2 }}
                                            @endif
                                        </td>
                                        <td
                                            class="{{ Config::get('style.rowCenterXs') }} text-xxs hidden sm:table-cell">
                                            <b>{{ $theYourParticipants->record_mode }}</b>
                                            {{-- @if ($theYourParticipants->number_your)
                                                <br>
                                                {!! nl2br(e($theYourParticipants->number_your)) !!}
                                            @endif --}}
                                        </td>
                                        <td
                                            class="{{ Config::get('style.rowCenterXs') }} text-xxs hidden sm:table-cell">
                                            {{ $theYourParticipants->team_enroller }}
                                        </td>
                                        <td
                                            class="{{ Config::get('style.rowCenterXs') }} text-xxs w-64 hidden sm:table-cell">
    ...
    $(document).ready(function() {
            ...
            $('.attendance-select-friday').on('change', function() {
                ...
                $.ajax({
                    url: '/actualizar_estado_y/' + participantId,
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
        ...
        function submitData() {
            ...
            $.ajax({
                url: "{{ route('YourParticipants.index') }}",
                method: "GET",
                data: {
    ...
    <script>
        $(document).ready(function() {
            $("#otrasede").on('input', function() {
                ...
                var training_id = document.getElementById('training_id').value;
                $.ajax({
                    url: "{{ route('ParticipantsOtraSedeYour.searchOtraSedeYour') }}",
                    type: 'GET',
                    dataType: 'json',
                    data: {
                        search: search,
