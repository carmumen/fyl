...
<body>
    <form action="{{ route('ProcessR.formRezagado') }}" method="POST">
        @csrf
        <input type="hidden" id="training_id_enroller" name="training_id_enroller" value=" {{ $participant->training_id_enroller }}" required>
        <input type="hidden" id="training_id" name="training_id" value=" {{ $participant->training_id }}" required/>
        <input type="hidden" id="DNI_enroller" name="DNI_enroller" value=" {{ old('DNI_enroller', $participant->DNI_enroller) }}" required>
        <input type="hidden" id="id" name="id" value=" {{ old('id', $participant->id) }}" required>
        <input type="hidden" id="DNI_old" name="DNI_old" value=" {{ old('DNI_old', $participant->DNI) }}" required>
        ...
        <input type="text" id="DNI" name="DNI" minlength="8" maxlength="13" placeholder="Cédula o pasaporte..."
            onkeypress="return valideKey(event);" value="<?php echo old('DNI', ($participant->nickname != '') ? $participant->DNI : ''); ?>" required />
        @error('DNI')
            <small style="color:red; font-weight:bold">{{ $message }}</small>
        @enderror

        <label for="names">Nombres:</label>
        <input type="text" id="names" name="names" value="<?php echo old('names', ($participant->nickname != '') ? $participant->names : ''); ?>" required oninput="this.value = this.value.toUpperCase();" />
        ...
        <input type="text" id="surnames" name="surnames" value="<?php echo old('surnames', ($participant->nickname != '') ? $participant->surnames : ''); ?>" required oninput="this.value = this.value.toUpperCase();" />
        @error('surnames')
            <small style="color:red; font-weight:bold">{{ $message }}</small>
        @enderror

        <label for="nickname">Nombre por el que prefiero que me llamen:</label>
        <input type="text" id="nickname" name="nickname" value=" {{ old('nickname', $participant->nickname) }}" required
        ...
        <select id="gender_catalog_id" name="gender_catalog_id" required>
            ...
            @foreach ($gender as $id => $name)
                <option value="{{ $id }}" @if ($id == old('gender_catalog_id', $participant->gender_catalog_id)) selected @endif>
                    {{ __($name) }}</option>
            @endforeach
        </select>
        ...
        <select id="civil_status_catalog_id" name="civil_status_catalog_id" required>
            ...
            @foreach ($civil_status as $id => $name)
                <option value="{{ $id }}" @if ($id == old('civil_status_catalog_id', $participant->civil_status_catalog_id)) selected @endif>
                    {{ __($name) }}</option>
            @endforeach
        </select>
        ...
        <label for="city_of_residenceT">Ciudad de Residencia:</label>
        <input type="text" id="city_of_residenceT" name="city_of_residenceT" value=" {{ old('city_of_residenceT', $participant->city_of_residenceT) }}" required
            oninput="this.value = this.value.toUpperCase();" />
        @error('city_of_residenceT')
            <small style="color:red; font-weight:bold">{{ $message }}</small>
        @enderror

        <label for="address">Dirección Domiciliaria:</label>
        <input type="text" id="address" name="address" value=" {{ old('address', $participant->address) }}" required
            oninput="this.value = this.value.toUpperCase();" />
        @error('address')
            <small style="color:red; font-weight:bold">{{ $message }}</small>
        @enderror
        ...
        <input type="text" id="email" name="email" value=" {{ old('email', $participant->email) }}" required
        ...
        <input type="text" id="phone2" name="phone2" 
            ...
            value=" {{ old('phone2', $participant->phone2) }}"
        ...
        <input type="text" id="occupation" name="occupation" value=" {{ old('occupation', $participant->occupation) }}" required
            oninput="this.value = this.value.toUpperCase();" />
        @error('occupation')
            <small style="color:red; font-weight:bold">{{ $message }}</small>
        @enderror
        ...
        <input type="text" id="emergency_contact" name="emergency_contact"
            value=" {{ old('emergency_contact', $participant->emergency_contact) }}" required oninput="this.value = this.value.toUpperCase();" />
        ...
        <input type="text" id="emergency_contact_phone" name="emergency_contact_phone"
            value=" {{ old('emergency_contact_phone', $participant->emergency_contact_phone) }}" onkeypress="return valideKey(event);" required />
        ...
        <select id="psychiatric_history" name="psychiatric_history" onchange="togglePsychiatricDetails();" required>
            ...
            <option value="SI" @if (old('psychiatric_history', $participant->psychiatric_history)) === 'SI') selected @endif>
                @lang('SI')
            </option>
            <option value="NO" @if (old('psychiatric_history', $participant->psychiatric_history)) === 'NO') selected @endif>
                @lang('NO')
            </option>
        </select>
        <input type="text" id="psychiatric_history_details" name="psychiatric_history_details"
            placeholder="Cuál?" value="{{ old('psychiatric_history_details', $participant->psychiatric_history_details) }}" style="display: none;" />
        ...
        <select id="medical_history" name="medical_history" onchange="toggleMedicalHistory();" required>
            <option value="">@lang('---')</option>
            <option value="SI" @if (old('medical_history', $participant->medical_history === 'SI')) selected @endif>
                @lang('SI')
            </option>
            <option value="NO" @if (old('medical_history', $participant->medical_history === 'NO')) selected @endif>
                @lang('NO')
            </option>
        </select>
        <input class="{{ Config::get('style.cajaTexto') }} w-full " type="text" id="medical_history_details"
            name="medical_history_details" placeholder="Cuál?" value="{{ old('medical_history_details', $participant->medical_history_details) }}"
            style="display: none;" />
        ...
        <select class="{{ Config::get('style.cajaTexto') }} w-24 mr-4" id="usual_medication" name="usual_medication"
            ...
            <option value="">@lang('---')</option>
            <option value="SI" @if ( old('usual_medication', $participant->usual_medication === 'SI')) selected @endif>
                @lang('SI')
            </option>
            <option value="NO" @if ( old('usual_medication', $participant->usual_medication === 'NO')) selected @endif>
                @lang('NO')
            </option>
        </select>
        <input type="text" id="usual_medication_details" name="usual_medication_details" placeholder="Cuál?"
            value="{{ old('usual_medication_details', $participant->usual_medication_details) }}" style="display: none;" />
