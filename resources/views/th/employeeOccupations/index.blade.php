<x-app-layout>
    ...
    <header>
        @include('partials/header', ['entidad' => 'EmployeeOccupations', 'pag' => $pag])
    </header>
                ...
                <div id="conResultados">
                    <table id="tablaDatos" class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-gray-100">
                            @foreach ($employeeOccupation as $employeeOccupations)
                                <tr class="border-b border-gray-200">
                                    <td class="{{ Config::get('style.rowCenter') }}">
                                        {{ $employeeOccupations->surnames }}
                                    </td>
                                    <td class="{{ Config::get('style.rowCenter') }}">
                                        {{ $employeeOccupations->names }}
                                    </td>
                                    <td class="{{ Config::get('style.rowCenter') }}">
                                        {{ $employeeOccupations->department }}
                                    </td>
                                    <td class="{{ Config::get('style.rowCenter') }}">
                                        {{ $employeeOccupations->jobTitle }}
                                    </td>
                                    <td class="{{ Config::get('style.rowCenter') }}">
                                        {{ __($employeeOccupations->status) }}
                                    </td>
                                    @auth
                                        <td class="w-24 inline-flex text-center py-1.5">
                                            <a class="{{ Config::get('style.btnEdit') }}"
                                                href="{{ route('EmployeeOccupations.edit', $employeeOccupations->id) }}">
                                                <span
                                                    class="icon-pencil text-orange-900 hover:bg-orange-500 hover:text-white"></span>
                                            </a>

                                            <form action="{{ route('EmployeeOccupations.destroy', $employeeOccupations) }}"
                                                method="POST">
                                                @csrf
                                                @method('DELETE')
